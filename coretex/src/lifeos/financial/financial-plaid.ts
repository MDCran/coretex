...
import { createLinkToken, plaidClient, plaidConfigured } from "./plaid-client.js";
import { provisionPlaidAccounts, syncPlaidItemFull } from "@/lib/financial/plaid-sync";
import { openIntegrationSecret, sealIntegrationSecret } from "../../security/integration-secret-box.js";
...
export async function exchangePlaidPublicToken(publicToken: string): Promise<{ itemId: string; sync: { added: number; updated: number; linked: number } }> {
    ...
    const plaidItem = existing
        ? await db.plaidItem.update({
              ...
              data: {
                  accessToken: sealIntegrationSecret(accessToken, { provider: "plaid", userId: user.id, field: "accessToken" }),
                  institutionId,
                  institutionName,
                  status: "good",
                  errorCode: null,
