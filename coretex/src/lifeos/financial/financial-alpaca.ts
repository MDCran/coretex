...
import { sealOptionalIntegrationSecret } from "../../security/integration-secret-box.js";
import {
    clearAlpacaCache,
    getAccount,
    getPortfolioHistory,
...
export async function ensureAlpacaFromEnv(userId: string): Promise<void> {
    ...
    await db.alpacaConnection.upsert({
        ...
        create: {
            userId,
            activePaper,
            paperApiKey: sealOptionalIntegrationSecret(paper?.apiKey, { provider: "alpaca", userId, field: "paperApiKey" }),
            paperApiSecret: sealOptionalIntegrationSecret(paper?.apiSecret, { provider: "alpaca", userId, field: "paperApiSecret" }),
            liveApiKey: sealOptionalIntegrationSecret(live?.apiKey, { provider: "alpaca", userId, field: "liveApiKey" }),
            liveApiSecret: sealOptionalIntegrationSecret(live?.apiSecret, { provider: "alpaca", userId, field: "liveApiSecret" }),
        },
        update: {
            // Env wins on refresh so newly added secrets in .env are picked up.
            paperApiKey: sealOptionalIntegrationSecret(paper?.apiKey ?? existing?.paperApiKey, { provider: "alpaca", userId, field: "paperApiKey" }),
            paperApiSecret: sealOptionalIntegrationSecret(paper?.apiSecret ?? existing?.paperApiSecret, { provider: "alpaca", userId, field: "paperApiSecret" }),
            liveApiKey: sealOptionalIntegrationSecret(live?.apiKey ?? existing?.liveApiKey, { provider: "alpaca", userId, field: "liveApiKey" }),
            liveApiSecret: sealOptionalIntegrationSecret(live?.apiSecret ?? existing?.liveApiSecret, { provider: "alpaca", userId, field: "liveApiSecret" }),
