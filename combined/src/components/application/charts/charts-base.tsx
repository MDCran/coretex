...
/**
 * Shared chart animation + curve constants so every Recharts surface reads
 * smoothly and consistently.
 *
 * - `CHART_CURVE` is the default interpolation for line/area series. `monotone`
 *   gives smooth curves that never overshoot the data (unlike `linear`'s hard
 *   corners). Step/journey charts that intentionally use `stepAfter` opt out.
 * - `chartAnimate()` resolves the reduced-motion preference at call time so the
 *   value is correct on the client after hydration. SSR-safe: defaults to
 *   animating on the server (where `window` is absent).
 * - `CHART_ANIM_DURATION` / `CHART_ANIM_EASING` keep entrance timing uniform.
 *
 * Most chart files inline the same `ANIMATE` module-const pattern; these
 * additive exports are available for new charts to avoid re-deriving it.
 */
export const CHART_CURVE = "monotone" as const;
export const CHART_ANIM_DURATION = 800;
export const CHART_ANIM_EASING = "ease-out" as const;

/** Whether charts should animate, honoring `prefers-reduced-motion`. SSR-safe. */
export const chartAnimate = (): boolean =>
