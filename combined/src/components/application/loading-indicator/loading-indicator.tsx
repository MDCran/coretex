...
export const LoadingIndicator = ({ type = "line-simple", size = "sm", label }: LoadingIndicatorProps) => {
    const renderSpinner = () => {
        if (type === "line-spinner") {
            return (
                <svg className={cx("animate-spin motion-reduce:animate-none", styles[size].spinner)} viewBox="0 0 32 32" fill="none">
                    <circle
        ...
        if (type === "dot-circle") {
            return (
                <svg className={cx("animate-spin text-fg-brand-primary motion-reduce:animate-none", styles[size].spinner)} viewBox="0 0 36 36" fill="none">
                    <path
