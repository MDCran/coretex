...
const SAFE_DATA_IMAGE = /^data:image\/(?:avif|gif|jpeg|png|webp);base64,[a-z0-9+/]+=*$/i;
...
export const Avatar = ({
    ...
    useEffect(() => setIsFailed(false), [safeSrc]);

    const canShowImage = safeSrc && !isFailed;

    const renderMainContent = () => {
        if (canShowImage) {
            return <img data-avatar-img className="size-full object-cover" src={safeSrc} alt={alt} onError={() => setIsFailed(true)} />;
        }
