    ...
    return (
        <AriaSlider {...rest} {...{ minValue, maxValue }} formatOptions={formatOptions ?? defaultFormatOptions}>
            ...
            <AriaSliderTrack className="relative h-6 w-full">
                {({ state }) => {
                    const { values, getThumbValue, getThumbPercent, getFormattedValue, isDisabled } = state;
                    const left = values.length === 1 ? 0 : getThumbPercent(0);
                    const width = values.length === 1 ? getThumbPercent(0) : getThumbPercent(1) - left;

                    return (
                        <>
                            <span className="absolute top-1/2 h-1.5 w-full -translate-y-1/2 rounded-full bg-quaternary" />
                            <span
                                className={cx("absolute top-1/2 h-1.5 w-full -translate-y-1/2 rounded-full bg-brand-solid", isDisabled && "opacity-40")}
                                style={{
                                    left: `${left * 100}%`,
                                    width: `${width * 100}%`,
                                }}
                                ...
                                return (
                                    <AriaSliderThumb
                                        ...
                                        className={({ isFocusVisible, isDragging, isDisabled: thumbDisabled }) =>
                                            cx(
                                                "top-1/2 box-border size-5 cursor-grab rounded-full bg-slider-handle-bg shadow-md ring-2 ring-slider-handle-border ring-inset transition duration-100 ease-out hover:scale-110 motion-reduce:transform-none",
                                                isFocusVisible && "outline-2 outline-offset-2 outline-focus-ring",
                                                isDragging && "scale-125 cursor-grabbing",
                                                thumbDisabled && "cursor-not-allowed opacity-40 hover:scale-100",
                                            )
                                        }
                                    >
