...
const ChartTooltip = ({ active, payload, label, money, unit }: { active?: boolean; payload?: any[]; label?: string; money?: boolean; unit?: string }) => {
    ...
    return (
        <div className="rounded-lg bg-primary px-3 py-2 text-xs shadow-lg ring-1 ring-secondary ring-inset">
            {showLabel ? <p className="mb-1 font-medium text-primary">{label}</p> : null}
            {payload.map((p: any) => (
...
const EmptyChart = ({ label, height }: { label: string; height: number }) => (
    <div className="flex items-center justify-center" style={{ height }}>
        <EmptyState size="sm">
            <EmptyState.Header />
            <EmptyState.Content>
                <EmptyState.Title>{label}</EmptyState.Title>
                <EmptyState.Description>Add some entries to see this chart.</EmptyState.Description>
            </EmptyState.Content>
        </EmptyState>
    </div>
);
...
export const TrendChart = ({ data, series, type = "line", height = 240, emptyLabel = "No data yet", money = true, unit, fitDomain = false }: TrendChartProps) => {
        ...
        <ResponsiveContainer width="100%" height={height}>
            {type === "bar" ? (
                <BarChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: -4 }}>
                    ...
                    <XAxis dataKey="label" {...axisProps} hide={sparse} />
