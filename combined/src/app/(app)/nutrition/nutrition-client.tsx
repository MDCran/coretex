...
import { Card, Field, NativeInput, NativeSelect, NativeTextarea, ProgressBar, SectionHeader } from "../health/_components/health-ui";
import { cx } from "@/utils/cx";
import { formatDate } from "@/lib/dates";
import { type UnitSystem, ozToMl, volumeToDisplay, volumeUnit, waterQuickAdds } from "@/lib/units";
import { MacroChip, MacroStat, formatMacroValue } from "./_components/macro-stat";
import { DeltaBadge } from "@/components/foundations/delta-badge";
import { MACRO_COLORS, MEAL_COLORS, type MacroKey } from "@/lib/nutrition/macro-colors";
import { NutritionDayNav, NutritionMonthNav } from "./_components/nutrition-date-nav";
import { QuickLogAi, type ChatLogTarget } from "./_components/quick-log-ai";
import { NutritionFactsCard } from "./_components/nutrition-facts-card";
import { FormModal } from "../health/_components/form-modal";
import { analyzeFoodChat, analyzeFoodPhoto, analyzeFoodText, discardPendingFoodPhoto, saveAnalyzedEntries, saveChatEntries, type AnalyzedItem } from "@/lib/actions/nutrition-ai";
import {
    addFoodEntry,
    addWater,
    applySavedMeal,
    ...
    updateFoodEntry,
    upsertFoodProduct,
} from "@/lib/actions/health-nutrition";
...
export function NutritionClient({
    ...
    useEffect(() => () => {
        ...
        // Do not delete here: navigation can unmount while a save action is
        // still linking this key. Explicit Cancel and stale analysis results
        // remain safe cleanup points.
    }, []);
    ...
    function discardConfirm() {
        const pendingKey = pendingPhotoKeyRef.current;
        pendingPhotoKeyRef.current = null;
        if (pendingKey) void discardPendingFoodPhoto(pendingKey).catch(() => {});
        clearConfirm();
    }

    function closePhotoAnalyzer() {
        photoRequestRef.current += 1;
        releasePhotoPreview();
        setAnalyzing(false);
        setPhotoOpen(null);
    ...
    async function runPhotoAnalyze(fd: FormData) {
        ...
        try {
            ...
            if (requestId !== photoRequestRef.current) {
                if (res.imageKey) await discardPendingFoodPhoto(res.imageKey).catch(() => {});
                releasePhotoPreview(preview);
                return;
            }
            setPhotoOpen(null);
            pendingPhotoKeyRef.current = res.imageKey ?? null;
            ...
            if (requestId === photoRequestRef.current) setAnalyzing(false);
        }
    }
    ...
    async function addCustomWater() {
        ...
        await wrap(() => addWater(fd), `Added ${formatMacroValue(n, waterUnitLabel as "oz" | "ml")} ${waterUnitLabel}`, () => setCustomWater(""));
    }
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <Card>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {([
                        ...
                        ["Water", volumeToDisplay(water, unitSystem), volumeToDisplay(waterGoalMl, unitSystem), volumeUnit(unitSystem) as "oz" | "ml", "water"],
            ...
            <Card>
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <MacroStat value={volumeToDisplay(water, unitSystem)} goal={volumeToDisplay(waterGoalMl, unitSystem)} unit={waterUnitLabel as "oz" | "ml"} />
                    </div>
            ...
            <FormModal
                isOpen={!!photoOpen}
                onOpenChange={(o) => { if (!o) closePhotoAnalyzer(); }}
            ...
            >
                <form action={runPhotoAnalyze} className="flex flex-col gap-4">
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        <Button color="secondary" type="button" onClick={closePhotoAnalyzer}>Cancel</Button>
            ...
            <FormModal
                isOpen={!!confirm}
                onOpenChange={(o) => { if (!o && !savingConfirm) discardConfirm(); }}
                ...
                {confirm && (
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            <Button color="secondary" type="button" onClick={discardConfirm} isDisabled={savingConfirm}>Cancel</Button>
    ...
    return (
        <Card>
            ...
            {goalCalories ? (
                ...
                <p className="mt-3 text-xs text-tertiary">Set a calorie goal to colour days by how you tracked. Volume shown in {volumeUnit(unitSystem) === "oz" ? "ounces" : "millilitres"} elsewhere.</p>
            )}
        </Card>
    );
}
