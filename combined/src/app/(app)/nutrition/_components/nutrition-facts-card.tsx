...
import { NutritionFactsLabel, type NutritionFacts } from "@/components/nutrition/nutrition-facts-label";
import { MACRO_COLORS, MEAL_COLORS, type MacroKey, type MealKey } from "@/lib/nutrition/macro-colors";
        ...
        <div className="rounded-2xl bg-primary p-5 ring-1 ring-secondary ring-inset sm:p-6">
            ...
            {view === "meal" && meals.length > 0 && (
                <select
                    ...
                    className="mt-3 w-full cursor-pointer rounded-lg bg-primary px-3 py-2 text-sm text-primary ring-1 ring-primary ring-inset transition focus:outline-2 focus:-outline-offset-2 focus:outline-brand"
                >
                    {meals.map((m) => (
                        <option key={m.mealType} value={m.mealType}>
                            {mealLabel(m.mealType)} · {m.entries.length} item{m.entries.length === 1 ? "" : "s"}
                        </option>
                    ))}
                </select>
            )}
            {view === "item" && itemsFlat.length > 0 && (
                ...
                >
                    {itemsFlat.map((x) => (
                        <option key={x.entry.id} value={x.entry.id}>
                            {x.entry.description} ({mealLabel(x.mealType)})
                        </option>
                    ))}
                </select>
            )}
