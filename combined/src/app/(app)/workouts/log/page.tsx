...
import { Activity as Dumbbell01 } from "@untitledui/icons";
import { requireUser } from "@/lib/auth";
import { db } from "@/lib/db";
import { dateKeyToUtc, setVolume, toDateKey } from "@/lib/workouts";
import { weightToDisplay, weightUnit } from "@/lib/units";
import { planStatus } from "@/lib/workouts-schedule";
                        ...
                        workouts.map((w) => {
                            ...
                            return (
                                <div key={w.id} className="flex items-center justify-between gap-3 rounded-lg p-4 ring-1 ring-secondary ring-inset">
                                    <Link href={`/workouts/session/${w.id}`} className="min-w-0 flex-1">
