...
const computeFitDomain = (data: TrendPoint[], series: SeriesDef[]): [number, number] | undefined => {
    ...
    // Don't dip below zero for non-negative series (scores, minutes, etc.).
...
interface TrendChartProps {
    ...
    height?: number;
    emptyLabel?: string;
    /** Fit the Y axis to the data (with padding) instead of starting at 0. Use for line trends of measurements; bars should stay zero-based. */
    fitDomain?: boolean;
}

/** Reusable recharts wrapper for learning trends. Renders an empty state when no data. */
export const TrendChart = ({ data, series, type = "bar", height = 240, emptyLabel = "No data yet", fitDomain = false }: TrendChartProps) => {
    if (!data.length) {
        return (
            <div className="flex items-center justify-center py-10">
                <EmptyState size="sm">
                    ...
                    <EmptyState.Content>
                        ...
                        <EmptyState.Description>Log some entries to see your trend.</EmptyState.Description>
                    </EmptyState.Content>
                </EmptyState>
            </div>
        );
