...
export default async function DocumentsPage() {
    ...
    const docs: DocumentCardData[] = documents.map((doc) => ({
        ...
        versions: doc.versions.map((v) => ({
            ...
            apps: [...v.usedAsResume, ...v.usedAsCoverLetter].map((a) => ({ id: a.id, role: a.role, companyName: a.company.name })),
        })),
    }));

    return (
        <div className="w-full">
            <PageHeader title="Documents" description="Versioned resumes, cover letters, and certifications — all in one place.">
                <NewDocumentDialog />
            </PageHeader>
            <DocumentsExplorer docs={docs} grouped={false} />
        </div>
    );
}
