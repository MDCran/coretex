                    ...
                    <Dialog aria-label="Manage tags">
                        <div className="w-full rounded-2xl bg-primary shadow-xl ring-1 ring-secondary">
                            ...
                            <div className="flex flex-col gap-4 p-5">
                                <p className="rounded-lg px-3 py-2 text-xs leading-relaxed text-tertiary" style={{ background: "var(--surface-2, rgba(120,120,120,0.08))" }}>
                                    Tags group contacts by how you know them — <span className="font-medium text-secondary">work</span>, <span className="font-medium text-secondary">gym</span>, <span className="font-medium text-secondary">family</span>, <span className="font-medium text-secondary">college</span>. Color-code them to filter your directory at a glance and spot each circle fast.
                                </p>
                    ...
                    <Dialog aria-label={editing ? "Edit tag" : "New tag"}>
                        <div className="w-full rounded-2xl bg-primary shadow-xl ring-1 ring-secondary">
                            ...
                            <form action={onSubmit} className="flex flex-col gap-4 p-5">
                                ...
                                <TextInput name="name" label="Name" isRequired defaultValue={editing?.name ?? ""} placeholder="e.g. Work, Gym, Family" />
                                <ColorSwatchPicker defaultValue={editing?.color ?? "brand"} sampleName={editing?.name ?? "Tag"} />
...
/** Visual color picker for a tag — clickable swatches + a live pill preview.
 *  Submits the chosen value through a hidden input so the server action is unchanged. */
function ColorSwatchPicker({ defaultValue, name = "color", sampleName }: { defaultValue?: string | null; name?: string; sampleName?: string }) {
    const [color, setColor] = useState(defaultValue ?? "brand");
    return (
        <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
                {COLORS.map((c) => (
                    <button
                        ...
                        className={cx(
                            "size-7 rounded-full ring-offset-2 ring-offset-primary transition",
                            TAG_SWATCH[c] ?? "bg-fg-quaternary",
                            color === c ? "scale-110 ring-2 ring-brand" : "ring-1 ring-secondary hover:scale-105",
                        )}
                    />
                ))}
            </div>
            <div className="mt-1 flex items-center gap-2">
                ...
                <span className={cx("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium text-white", TAG_SWATCH[color] ?? "bg-fg-quaternary")}>
