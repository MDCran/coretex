...
import { Plus, PieChart03, Edit01, Trash02, ChevronLeft, ChevronRight } from "@untitledui/icons";
...
export interface BudgetMonthEntry {
    ...
    totalBudget: number;
    totalSpent: number;
    categories: Array<{ id: string; spent: number }>;
}
...
/** Trailing budget-vs-actual chart with a category filter and a scrubbable six-month window. */
function BudgetVsActualSection({ history, categories }: { history: BudgetMonthEntry[]; categories: CategoryRow[] }) {
    ...
    const chartData = useMemo<TrendPoint[]>(() => {
        if (!chartCategoryId) {
            return history.map((entry) => ({ label: monthLabel(entry.month), Budget: entry.totalBudget, Actual: entry.totalSpent }));
        }
        const category = categories.find((candidate) => candidate.id === chartCategoryId);
        const budget = category?.monthlyBudget ?? 0;
        return history.map((entry) => ({
            label: monthLabel(entry.month),
            Budget: budget,
            Actual: entry.categories.find((candidate) => candidate.id === chartCategoryId)?.spent ?? 0,
        }));
    }, [history, categories, chartCategoryId]);

    const chartSlice = chartData.slice(offset, offset + windowSize);
    const selected = history[Math.min(history.length - 1, offset + windowSize - 1)];
    const labelStart = chartSlice[0]?.label ?? "—";
    const labelEnd = chartSlice.at(-1)?.label ?? "—";
    const monthRows = useMemo(() => {
        if (!selected) return [];
        return categories
            .map((category) => {
                const spent = selected.categories.find((candidate) => candidate.id === category.id)?.spent ?? 0;
                const budget = category.monthlyBudget ?? 0;
    ...
    return (
        <Card className="flex flex-col gap-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h3 className="text-sm font-semibold text-primary">Budget vs. actual</h3>
                    <p className="text-xs text-tertiary">Trailing {history.length} months · showing {labelStart} – {labelEnd}.</p>
                </div>
                <div className="flex items-center gap-2">
                    <NativeSelect value={chartCategoryId} onChange={(event) => setChartCategoryId(event.target.value)} className="w-48">
                        <option value="">All categories</option>
                        {categories.map((category) => (
                            <option key={category.id} value={category.id}>{category.name}</option>
                        ))}
                    </NativeSelect>
                    <button type="button" className="grid size-8 place-items-center rounded-lg border border-secondary text-tertiary transition hover:bg-secondary disabled:opacity-40" disabled={offset <= 0} onClick={() => setOffset((value) => Math.max(0, value - 1))} aria-label="Earlier months">
                        <ChevronLeft className="size-4" />
                    </button>
                    <button type="button" className="grid size-8 place-items-center rounded-lg border border-secondary text-tertiary transition hover:bg-secondary disabled:opacity-40" disabled={offset >= maxOffset} onClick={() => setOffset((value) => Math.min(maxOffset, value + 1))} aria-label="Later months">
                        <ChevronRight className="size-4" />
                    </button>
                </div>
            </div>

            <TrendChart data={chartSlice} series={[{ key: "Budget", name: "Budget", color: "var(--color-brand-500)" }, { key: "Actual", name: "Actual", color: "var(--color-utility-orange-500)" }]} type="bar" height={240} money emptyLabel="No budget history yet" />

            {selected && (
                <div className="flex flex-col gap-3 border-t border-secondary pt-4">
                    <p className="text-center text-sm font-medium text-primary">{monthLabel(selected.month)} adherence</p>
                    {monthRows.length === 0 ? (
                        <p className="py-4 text-center text-sm text-tertiary">No budgeted spending recorded for {monthLabel(selected.month)}.</p>
                    ) : (
                            ...
                            {monthRows.map((row) => {
                                ...
                                return (
                                    <li key={row.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                                        <span className="flex items-center gap-2 text-secondary"><CategoryDot color={row.color} />{row.name}</span>
                                        <span className="flex items-center gap-2">
                                            <span className={over ? "font-medium text-error-primary" : "font-medium text-primary"}>{formatCurrency(row.spent)}</span>
                                            <span className="text-tertiary">of {row.budget > 0 ? formatCurrency(row.budget) : "no budget"}</span>
                                            {over && <span className="rounded-full bg-error-primary px-2 py-0.5 text-xs font-medium text-error-primary">Over</span>}
                                        </span>
                                    </li>
                                );
                            })}
...
export function BudgetClient({
    ...
    const createMutation = useLifeOSMutation(client, "financial:createBudgetCategory");
    const updateMutation = useLifeOSMutation(client, "financial:updateBudgetCategory");
    const deleteMutation = useLifeOSMutation(client, "financial:deleteBudgetCategory");
    const genericMutation = useLifeOSMutation(client, "financial:setGenericBudgetTotal");
    ...
    async function onSubmit(fd: FormData) {
        const payload = {
            ...
            monthlyBudget: String(fd.get("monthlyBudget") ?? ""),
        };
        try {
    ...
    async function onGenericSubmit(fd: FormData) {
        try {
            await genericMutation.mutate({ total: String(fd.get("total") ?? "") });
        ...
        <div className="flex flex-col gap-6">
            ...
            {uncategorizedSpend > 0 && (
                <Card className="flex flex-wrap items-center justify-between gap-2 border border-warning-secondary bg-warning-primary py-3">
                    ...
                    <Button size="sm" color="secondary" onClick={() => onNavigateTab?.("transactions")} iconLeading={Plus}>
                        Categorize transactions
                    </Button>
                </Card>
            )}

            <BudgetVsActualSection history={history} categories={categories} />
            ...
            >
                {drillCategory && (
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            <Button size="sm" color="secondary" onClick={() => onNavigateTab?.("transactions")}>
