...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // Newer react-aria-components types allow multi-select calendar values; this context is single-select only.
    const handleChange = (newValue: DateValue | readonly DateValue[]) => {
        onChange(Array.isArray(newValue) ? ((newValue[0] as DateValue | undefined) ?? null) : (newValue as DateValue));
    };
                ...
                {({ state }) => (
                    <>
                        <header className="flex items-center justify-between">
                            <Button slot="previous" aria-label="Previous month" iconLeading={ChevronLeft} size="sm" color="tertiary" className="size-8" />
                            <AriaHeading className="text-sm font-semibold text-fg-secondary" />
                            <Button slot="next" aria-label="Next month" iconLeading={ChevronRight} size="sm" color="tertiary" className="size-8" />
                        </header>
