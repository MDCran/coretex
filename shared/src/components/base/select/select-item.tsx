...
interface SelectItemProps extends Omit<AriaListBoxItemProps<SelectItemType>, "id">, SelectItemType {
    ...
    /** Place supporting text beside or below the primary label. */
    supportingTextPosition?: "inline" | "below";
}
...
export const SelectItem = ({
    ...
    const isStacked = Boolean(supportingText) && supportingTextPosition === "below";
    ...
    return (
        ...
        >
            {(state) => (
                <div
                    className={cx(
                        "flex cursor-pointer rounded-md outline-hidden select-none",
                ...
                >
                    ...
                    <div
                        className={cx(
                            ...
                            isStacked ? "flex-col items-start gap-y-0.5" : "flex-wrap",
                    ...
                    >
                        ...
                        {supportingText && (
                            <AriaText
                                slot="description"
                                className={cx(
                                    "max-w-full text-tertiary",
                                    isStacked ? "w-full truncate" : "whitespace-nowrap",
                                    sizes[size].text,
                                )}
                            >
                                {supportingText}
                    ...
                    {state.isSelected && selectionIndicator === "checkmark" && (
                        <Check aria-hidden="true" className={cx("ml-auto shrink-0 text-fg-brand-primary", isStacked && "mt-0.5", sizes[size].check)} />
                    )}
