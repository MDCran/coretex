...
/**
 * Shared option shape for both the raw {@link NativeSelect} and the Untitled UI
 * popover-based `RichSelect`. Only `label`/`value` are required — the rich fields
 * (icon, supportingText, …) are rendered by `RichSelect` and ignored by NativeSelect.
 */
export interface RichSelectOption {
    value: string;
    label: string;
    /** Secondary line shown beneath/next to the label in rich dropdowns. */
    supportingText?: string;
    /** Tertiary detail line (rich dropdowns only). */
    detailText?: string;
    /** Native `title` tooltip for the row. */
    hint?: string;
    /** Leading icon component or element. */
    icon?: ReactNode;
    /** Leading avatar image URL (takes precedence over `icon`). */
    avatarUrl?: string;
    disabled?: boolean;
}
